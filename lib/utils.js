import { RichText } from "@atproto/api";
import { htmlToText } from "html-to-text";
import { JSDOM } from "jsdom";
import sharp from "sharp";

const AT_URI = /at:\/\/(?<did>did:[^/]+)\/(?<type>[^/]+)\/(?<rkey>[^/]+)/;

/**
 * Extract links from HTML content with display text and URL
 * @param {string} html - HTML content
 * @returns {Array<{text: string, url: string}>} Array of link objects
 */
export function extractHtmlLinks(html) {
  if (!html) return [];
  const linkRegex =
    /<a\s+[^>]*href=["'](https?:\/\/[^"']+)["'][^>]*>(.*?)<\/a>/gi;
  const links = [];
  let match;
  while ((match = linkRegex.exec(html)) !== null) {
    const url = match[1];
    const text = match[2].replaceAll(/<[^>]+>/g, "").trim();
    if (text && url) {
      links.push({ text, url });
    }
  }
  return links;
}

/**
 * Build Bluesky link facets mapping display text to URLs.
 * Only creates facets where the display text differs from the URL
 * (plain URLs are auto-detected by RichText.detectFacets).
 * @param {string} text - Final post text
 * @param {Array<{text: string, url: string}>} links - Link objects from HTML
 * @returns {Array<object>} Bluesky facet objects
 */
export function buildLinkFacets(text, links) {
  if (!text || !links || links.length === 0) return [];

  const encoder = new TextEncoder();
  const facets = [];
  let searchFrom = 0;

  for (const link of links) {
    if (link.text === link.url) continue;

    const index = text.indexOf(link.text, searchFrom);
    if (index === -1) continue;

    const byteStart = encoder.encode(text.slice(0, index)).byteLength;
    const byteEnd = byteStart + encoder.encode(link.text).byteLength;

    facets.push({
      index: { byteStart, byteEnd },
      features: [
        {
          $type: "app.bsky.richtext.facet#link",
          uri: link.url,
        },
      ],
    });

    searchFrom = index + link.text.length;
  }

  return facets;
}

/**
 * Convert plain text to rich text, optionally merging manual link facets
 * @param {import("@atproto/api").Agent} client - AT Protocol agent
 * @param {string} text - Text to convert
 * @param {Array<object>} [extraFacets] - Additional facets to merge
 * @returns {Promise<RichText>} Rich text
 */
export const createRichText = async (client, text, extraFacets = []) => {
  const rt = new RichText({ text });
  await rt.detectFacets(client);

  if (extraFacets.length > 0) {
    const existingRanges = new Set(
      (rt.facets || []).map((f) => `${f.index.byteStart}-${f.index.byteEnd}`),
    );

    for (const facet of extraFacets) {
      const key = `${facet.index.byteStart}-${facet.index.byteEnd}`;
      if (!existingRanges.has(key)) {
        if (!rt.facets) rt.facets = [];
        rt.facets.push(facet);
      }
    }
  }

  return rt;
};

/**
 * Get post parts (UID and CID)
 * @param {string} url - Post URL
 * @returns {object} Parts
 */
export const getPostParts = (url) => {
  const pathParts = new URL(url).pathname.split("/");
  const did = pathParts[2];
  const rkey = pathParts[4];
  return { did, rkey };
};

/**
 * Convert Bluesky URI to post URL
 * @param {string} profileUrl - Profile URL
 * @param {string} uri - Bluesky URI
 * @returns {string|undefined} Post URL
 */
export const uriToPostUrl = (profileUrl, uri) => {
  const match = uri.match(AT_URI);
  if (match) {
    let { did, rkey, type } = match.groups;
    type = type.split(".").at(-1);
    return `${profileUrl}/${did}/${type}/${rkey}`;
  }
};

/**
 * Get hashtags for given categories
 *
 * Uses the last segment of a hierarchical category, and removes any characters
 * Bluesky doesn’t recognise as part of a tag, so that `holidays/family trips`
 * becomes `#familytrips`. Tags longer than 64 characters are dropped, as
 * `detectFacets` ignores them.
 * @param {Array|string} [category] - JF2 `category` property
 * @returns {Array} Hashtags
 */
export const createHashtags = (category) => {
  if (!category) {
    return [];
  }

  const categories = Array.isArray(category) ? category : [category];
  const hashtags = [];

  for (const item of categories) {
    if (typeof item !== "string") {
      continue;
    }

    const name = item
      .split("/")
      .at(-1)
      .replaceAll(/[^\p{L}\p{N}_]/gu, "");
    const hashtag = `#${name}`;

    if (name && name.length <= 64 && !hashtags.includes(hashtag)) {
      hashtags.push(hashtag);
    }
  }

  return hashtags;
};

/**
 * Get post text from given JF2 properties (legacy, used for likes/bookmarks)
 * @param {object} properties - JF2 properties
 * @param {boolean} [includePermalink] - Include permalink in post
 * @param {boolean} [includeCategories] - Add categories as hashtags
 * @returns {string} Post text
 */
export const getPostText = (
  properties,
  includePermalink,
  includeCategories,
) => {
  let text = "";

  if (properties.name && properties.name !== "") {
    text = `${properties.name} ${properties.url}`;
  } else if (properties.content?.html) {
    text = htmlToStatusText(properties.content.html);
  } else if (properties.content?.text) {
    text = properties.content.text;
  }

  // Show hashtags at the end of a post, where Bluesky links them. Skip any
  // already written into the post content. `createRichText` runs
  // `detectFacets`, which turns them into tag facets.
  const hashtags = includeCategories
    ? createHashtags(properties.category).filter(
        (hashtag) =>
          !new RegExp(String.raw`${hashtag}(?![\p{L}\p{N}_])`, "iu").test(text),
      )
    : [];
  const suffix = hashtags.length > 0 ? `\n\n${hashtags.join(" ")}` : "";
  const limit = 300 - suffix.length;

  // Truncate status if longer than the limit
  // ALWAYS include permalink when truncating so readers can see full post
  if (text.length > limit) {
    const permalinkSuffix = `\n\n${properties.url}`;
    const maxLength = limit - permalinkSuffix.length - 3;
    text = text.slice(0, maxLength).trim() + "..." + permalinkSuffix;
  } else if (includePermalink && !text.includes(properties.url)) {
    text += `\n\n${properties.url}`;
  }

  return text + suffix;
};

/**
 * Extract plain text content from properties without appending URLs
 * @param {object} properties - JF2 properties
 * @returns {string} Plain text content
 */
export function getContentText(properties) {
  if (properties.name && properties.name !== "") {
    return properties.name;
  }
  if (properties.content?.html) {
    return htmlToText(properties.content.html, {
      selectors: [
        { selector: "a", options: { ignoreHref: true } },
        { selector: "img", format: "skip" },
      ],
      wordwrap: false,
    });
  }
  if (properties.content?.text) {
    return properties.content.text;
  }
  return "";
}

/**
 * Remove a URL from text, cleaning up surrounding whitespace and prefixes
 * @param {string} text - Text containing the URL
 * @param {string} url - URL to remove
 * @returns {string} Cleaned text
 */
export function removeUrlFromText(text, url) {
  if (!text || !url) return text;
  let result = text.replace(url, "");
  // Clean up common prefixes left behind when URL was at the end
  result = result.replace(
    /\s*(Réf|Ref|ref|via|Via|source|Source|link|Link|→|—)\s*$/im,
    "",
  );
  // Clean up excessive whitespace
  result = result
    .replaceAll(/\n{3,}/g, "\n\n")
    .replaceAll(/[ \t]{2,}/g, " ")
    .trim();
  return result;
}

/**
 * Build post text for Bluesky syndication with proper URL and character handling.
 *
 * Two modes:
 * 1. External URL exists → remove from text (shown as OG card), append permalink as text
 * 2. No external URL → content only (permalink shown as OG card)
 * @param {object} properties - JF2 properties
 * @param {object} [options] - Options
 * @param {string} [options.externalUrl] - External URL that will be used as OG card
 * @param {boolean} [options.includeCategories] - Append categories as hashtags
 * @returns {string} Post text fitting within 300 chars
 */
export function buildPostText(properties, options = {}) {
  const { externalUrl, includeCategories } = options;

  let text = getContentText(properties);

  // Hashtags go last, so reserve their room before truncating
  const hashtags = includeCategories
    ? createHashtags(properties.category).filter(
        (hashtag) =>
          !new RegExp(String.raw`${hashtag}(?![\p{L}\p{N}_])`, "iu").test(text),
      )
    : [];
  const hashtagSuffix = hashtags.length > 0 ? `\n\n${hashtags.join(" ")}` : "";
  const limit = 300 - hashtagSuffix.length;

  if (externalUrl) {
    // External URL will be shown as OG card — remove from text to save space
    text = removeUrlFromText(text, externalUrl);

    // Always append permalink to original post
    const permalink = properties.url;
    if (permalink) {
      const suffix = `\n\n${permalink}`;
      const available = limit - suffix.length;
      if (text.length > available) {
        text = text.slice(0, available - 1).trim() + "\u{2026}";
      }
      return (text + suffix).trim() + hashtagSuffix;
    }
  }

  // No external URL — permalink will be shown as OG card
  // No need to duplicate it in text
  if (text.length > limit) {
    text = text.slice(0, limit - 1).trim() + "\u{2026}";
  }

  return text.trim() + hashtagSuffix;
}

/**
 * Get post text for a like of an external URL (Bluesky version).
 * The external URL is shown as an OG card embed, so the text contains
 * commentary + blog permalink (for webmentions), not the liked URL.
 * @param {object} properties - JF2 properties
 * @param {string} likedUrl - The URL being liked
 * @returns {string} Post text
 */
export const getLikePostText = (properties, likedUrl) => {
  let text = "";

  // Get the content/comment
  if (properties.content?.html) {
    text = htmlToStatusText(properties.content.html);
  } else if (properties.content?.text) {
    text = properties.content.text;
  }

  // Remove the liked URL from text if already present (OG card shows it)
  if (text && likedUrl) {
    text = text.replace(likedUrl, "").trim();
  }

  // Append blog permalink for webmentions
  const permalink = properties.url;
  if (permalink) {
    if (text) {
      text += `\n\n${permalink}`;
    } else {
      text = `❤️ ${permalink}`;
    }
  } else if (!text) {
    // Fallback: no content, no permalink — use liked URL
    text = `❤️ ${likedUrl}`;
  }

  // Truncate if needed (Bluesky limit is 300 chars)
  if (text.length > 300) {
    const suffix = permalink ? `\n\n${permalink}` : `\n\n❤️ ${likedUrl}`;
    const maxLength = 300 - suffix.length - 3;
    const contentPart = text.replace(suffix, "").slice(0, maxLength).trim();
    text = contentPart + "..." + suffix;
  }

  return text;
};

/**
 * Get post text for a repost of an external URL (Bluesky version).
 * The external URL is shown as an OG card embed, so the text contains
 * commentary + blog permalink (for webmentions), not the reposted URL.
 * @param {object} properties - JF2 properties
 * @param {string} repostUrl - The URL being reposted
 * @returns {string} Post text
 */
export const getRepostPostText = (properties, repostUrl) => {
  let text = "";

  // Get the content/comment
  if (properties.content?.html) {
    text = htmlToStatusText(properties.content.html);
  } else if (properties.content?.text) {
    text = properties.content.text;
  }

  // Remove the reposted URL from text if already present (OG card shows it)
  if (text && repostUrl) {
    text = text.replace(repostUrl, "").trim();
  }

  // Append blog permalink for webmentions
  const permalink = properties.url;
  if (permalink) {
    if (text) {
      text += `\n\n${permalink}`;
    } else {
      text = `🔁 ${permalink}`;
    }
  } else if (!text) {
    // Fallback: no content, no permalink — use repost URL
    text = `🔁 ${repostUrl}`;
  }

  // Truncate if needed (Bluesky limit is 300 chars)
  if (text.length > 300) {
    const suffix = permalink ? `\n\n${permalink}` : `\n\n🔁 ${repostUrl}`;
    const maxLength = 300 - suffix.length - 3;
    const contentPart = text.replace(suffix, "").slice(0, maxLength).trim();
    text = contentPart + "..." + suffix;
  }

  return text;
};

/**
 * Get post text for a bookmark of an external URL (Bluesky version).
 * The external URL is shown as an OG card embed, so the text contains
 * commentary + blog permalink (for webmentions), not the bookmarked URL.
 * @param {object} properties - JF2 properties
 * @param {string} bookmarkUrl - The URL being bookmarked
 * @returns {string} Post text
 */
export const getBookmarkPostText = (properties, bookmarkUrl) => {
  let text = "";

  // Get the content/comment
  if (properties.name && properties.name !== "") {
    text = properties.name;
  } else if (properties.content?.html) {
    text = htmlToStatusText(properties.content.html);
  } else if (properties.content?.text) {
    text = properties.content.text;
  }

  // Remove the bookmarked URL from text if already present (OG card shows it)
  if (text && bookmarkUrl) {
    text = text.replace(bookmarkUrl, "").trim();
  }

  // Append blog permalink for webmentions
  const permalink = properties.url;
  if (permalink) {
    if (text) {
      if (!text.includes(permalink)) {
        text += `\n\n${permalink}`;
      }
    } else {
      text = `🔖 ${permalink}`;
    }
  } else if (!text) {
    // Fallback: no content, no permalink — use bookmarked URL
    text = `🔖 ${bookmarkUrl}`;
  }

  // Truncate if needed (Bluesky limit is 300 chars)
  if (text.length > 300) {
    const suffix = permalink ? `\n\n${permalink}` : `\n\n🔖 ${bookmarkUrl}`;
    const maxLength = 300 - suffix.length - 3;
    const contentPart = text.replace(suffix, "").slice(0, maxLength).trim();
    text = contentPart + "..." + suffix;
  }

  return text;
};

/**
 * Constrain image buffer to be under 1MB
 * @param {Buffer} buffer - Image buffer
 * @param {number} maxBytes - Maximum byte length
 * @param {number} [quality] - Image quality
 * @returns {Promise<Buffer>} Compressed image
 */
export async function constrainImage(buffer, maxBytes, quality = 90) {
  let compressed = await sharp(buffer).jpeg({ quality }).toBuffer();

  while (compressed.byteLength > maxBytes) {
    quality -= 5;
    compressed = await sharp(buffer).jpeg({ quality }).toBuffer();
  }
  return compressed;
}

/**
 * Compress image buffer to be under 1MB for Bluesky
 * @param {Buffer} buffer - Image buffer
 * @param {string} mimeType - Original MIME type
 * @returns {Promise<{buffer: Buffer, mimeType: string}>} Compressed image
 */
export async function getPostImage(buffer, mimeType) {
  const MAX_SIZE = 1_000_000; // Bluesky's upload limit is 1,000,000 bytes (not 1 MiB)
  if (buffer.length < MAX_SIZE) {
    return { buffer, mimeType };
  }
  const compressed = await constrainImage(buffer, MAX_SIZE);
  return { buffer: compressed, mimeType: "image/jpeg" };
}

/**
 * Convert HTML to plain text, appending all link hrefs not already in text
 * @param {string} html - HTML
 * @returns {string} Text
 */
export const htmlToStatusText = (html) => {
  const hrefs = [...html.matchAll(/href="(https?:\/\/.+?)"/g)];
  const urls = [...new Set(hrefs.map((h) => h[1]))];

  const text = htmlToText(html, {
    selectors: [
      { selector: "a", options: { ignoreHref: true } },
      { selector: "img", format: "skip" },
    ],
    wordwrap: false,
  });

  // Append URLs that aren't already visible in the text
  const missingUrls = urls.filter((url) => !text.includes(url));
  if (missingUrls.length > 0) {
    return `${text}\n\n${missingUrls.join("\n")}`;
  }
  return text;
};

/**
 * Fetch OpenGraph metadata from a URL
 * @param {string} url - URL to fetch OG data from
 * @returns {Promise<{title: string, description: string, imageUrl: string|null}>} OG metadata
 */
export async function fetchOpenGraphData(url) {
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 10_000);
    const response = await fetch(url, {
      headers: {
        "User-Agent":
          "Mozilla/5.0 (compatible; Indiekit/1.0; +https://getindiekit.com)",
        Accept: "text/html,application/xhtml+xml",
      },
      redirect: "follow",
      signal: controller.signal,
    });
    clearTimeout(timeoutId);

    if (!response.ok) {
      return { title: url, description: "", imageUrl: undefined };
    }

    const html = await response.text();
    // Strip <style> tags before parsing to prevent jsdom CSS parsing errors
    const cleanHtml = html.replaceAll(/<style[\s\S]*?<\/style>/gi, "");
    const dom = new JSDOM(cleanHtml, { url });
    const document = dom.window.document;

    // Get OG title, fallback to regular title
    const ogTitle =
      document
        .querySelector('meta[property="og:title"]')
        ?.getAttribute("content") ||
      document.querySelector("title")?.textContent ||
      url;

    // Get OG description, fallback to meta description
    const ogDescription =
      document
        .querySelector('meta[property="og:description"]')
        ?.getAttribute("content") ||
      document
        .querySelector('meta[name="description"]')
        ?.getAttribute("content") ||
      "";

    // Get OG image
    let ogImage =
      document
        .querySelector('meta[property="og:image"]')
        ?.getAttribute("content") || undefined;

    // Handle relative URLs for image
    if (ogImage && !ogImage.startsWith("http")) {
      const baseUrl = new URL(url);
      ogImage = new URL(ogImage, baseUrl.origin).href;
    }

    return {
      title: ogTitle.slice(0, 300), // Bluesky title limit
      description: ogDescription.slice(0, 1000), // Reasonable limit
      imageUrl: ogImage,
    };
  } catch (error) {
    console.error(`Failed to fetch OG data for ${url}:`, error);
    return { title: url, description: "", imageUrl: undefined };
  }
}

/**
 * Extract URLs from text using a comprehensive regex
 * @param {string} text - Text to search for URLs
 * @returns {Array<string>} Array of URLs found
 */
function extractUrlsFromText(text) {
  if (!text) return [];
  // Match URLs starting with http:// or https://
  // This regex is more permissive to catch URLs in plain text
  const urlRegex = /https?:\/\/[^\s<>"')\]]+/gi;
  const matches = text.match(urlRegex) || [];
  // Clean up trailing punctuation that might have been captured
  return matches.map((url) =>
    url.replace(/[.,;:!?)]+$/, "").replace(/\)+$/, ""),
  );
}

/**
 * Extract the primary URL from post properties
 * @param {object} properties - JF2 properties
 * @param {string} [ownDomain] - Own domain to deprioritize (e.g., "rmendes.net")
 * @returns {string|null} Primary URL to create card for
 */
export function getExternalUrl(properties, ownDomain) {
  // For likes, use the liked URL
  if (properties["like-of"]) {
    return properties["like-of"];
  }

  // For bookmarks, use the bookmarked URL
  if (properties["bookmark-of"]) {
    return properties["bookmark-of"];
  }

  // For replies, use the in-reply-to URL
  if (properties["in-reply-to"]) {
    return properties["in-reply-to"];
  }

  // Collect all URLs from content
  let urls = [];

  // Extract from HTML href attributes (both single and double quotes)
  if (properties.content?.html) {
    const hrefMatches = [
      ...properties.content.html.matchAll(/href=["'](https?:\/\/[^"']+)["']/gi),
    ];
    urls.push(...hrefMatches.map((m) => m[1]));
  }

  // Extract plain text URLs from HTML content (for URLs not in anchors)
  if (properties.content?.html) {
    const plainUrls = extractUrlsFromText(properties.content.html);
    urls.push(...plainUrls);
  }

  // Extract from plain text content
  if (properties.content?.text) {
    const textUrls = extractUrlsFromText(properties.content.text);
    urls.push(...textUrls);
  }

  // Deduplicate URLs
  urls = [...new Set(urls)];

  if (urls.length === 0) {
    return;
  }

  // If we have an ownDomain, only return truly external URLs.
  // Own-domain links (e.g., linking to your own bookmark post) should not
  // steal the OG card — let the note's own URL be used instead.
  if (ownDomain) {
    return urls.find((url) => {
      try {
        const hostname = new URL(url).hostname;
        return !hostname.includes(ownDomain);
      } catch {
        return true;
      }
    });
  }

  // Return the first URL found (user arranges links with primary first)
  return urls[0];
}
