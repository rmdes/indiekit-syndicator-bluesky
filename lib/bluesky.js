...
export class Bluesky {
  ...
  async uploadImageFromUrl(imageUrl) {
    ...
    try {
      ...
      console.error(
        `Failed to upload OG image: ${error instanceof Error ? error.message : String(error)}`,
      );
      return;
    }
  }
   ...
   */
  async createExternalEmbed(url, { me } = {}) {
    if (!url) return;
    ...
    try {
      ...
      const slug = isOwnPost
        ? this.#urlToOgSlug(new URL(url).pathname)
        : undefined;

      // The OG metadata is always needed for title/description, so fetch it
      // alongside the image upload rather than after it.
      let [thumb, ogData] = await Promise.all([
        slug ? this.uploadImageFromUrl(`${meNorm}/og/${slug}.png`) : undefined,
      ...
      console.error(
        `Failed to create external embed: ${error instanceof Error ? error.message : String(error)}`,
      );
      return;
    }
  }
